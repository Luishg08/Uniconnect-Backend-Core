...
**Prompt Sugerido:**
Crea el archivo `src/notifications/domain/strategy/interfaces.ts` con la interfaz `INotificacionStrategy` que expone el atribudocumentar patrón CoR en README con diagramas UML y de secuenciato readonly `canal: string` y el método `enviar(notificacion: NotificacionDTO): Promise<ResultadoEnvio>`. Define también en el mismo archivo la interfaz `NotificacionDTO` con los campos `id_user`, `mensaje`, `tipo_evento`, `entidad_relacionada_id?` y `metadata?`, y la interfaz `ResultadoEnvio` con `canal`, `exitoso`, `error?` y `timestamp`. Crea además `src/notifications/notifications.tokens.ts` que exporte la constante `NOTIFICACION_STRATEGIES = 'NOdocumentar patrón CoR en README con diagramas UML y de secuenciaTIFICACION_STRATEGIES'` para usar como token de inyección.
